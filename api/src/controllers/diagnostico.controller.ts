...
export class DiagnosticoController {
  ...
  async list() {
    ...
    return diagnostico.map(diagnostico => DiagnosticoViewModel.toHTTP(diagnostico));
  }

  @Get('client/:clientId')
  async findDiagnosticoByClientId(@Param('clientId') clientId: string) {
    const { diagnostico } = await this.diagnosticoService.findDiagnosticoByClientId(clientId);
    return DiagnosticoViewModel.toHTTP(diagnostico);
  }
}
